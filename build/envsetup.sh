function __print_hash_functions_help() {
cat <<EOF
Additional HashOS functions:
- breakfast:       Setup the build environment, but only list
                   devices we support.
- brunch:          Sets up build environment using breakfast(),
                   and then comiles using mka() against banana target.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the hash model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch hash_$target-$variant
        fi
    fi
    return $?
}
...
function repopick() {
    ...
    T=$(gettop)
    $T/vendor/hash/build/tools/repopick.py $@
}
